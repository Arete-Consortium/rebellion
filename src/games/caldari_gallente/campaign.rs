...
impl CGBossType {
    ...
    pub fn dialogue_intro(&self) -> &'static str {
        match self {
            CGBossType::PatrolCommander => {
                "You dare violate Caldari space? The State will crush you."
            }
            CGBossType::FleetCommander => {
                "Federation dogs! You will learn to fear the Caldari Navy."
            }
            CGBossType::EliteSquadron => {
                "We are the finest pilots in New Eden. You face certain death."
            }
    ...
    pub fn type_id(&self, enemy_faction: crate::core::Faction) -> u32 {
        ...
        match (self, enemy_faction) {
            ...
            (CGBossType::PatrolCommander, Faction::Caldari) => 16238, // Cormorant destroyer
            (CGBossType::FleetCommander, Faction::Caldari) => 16227,  // Ferox (battlecruiser)
            (CGBossType::EliteSquadron, Faction::Caldari) => 24698,   // Drake (battlecruiser)
            (CGBossType::FleetAdmiral, Faction::Caldari) => 638,      // Raven (battleship)
            // Gallente bosses (player is Caldari, fighting Gallente)
            (CGBossType::PatrolCommander, Faction::Gallente) => 16240, // Catalyst destroyer
            (CGBossType::FleetCommander, Faction::Gallente) => 24700,  // Myrmidon (battlecruiser)
            (CGBossType::EliteSquadron, Faction::Gallente) => 641,     // Megathron (battleship)
