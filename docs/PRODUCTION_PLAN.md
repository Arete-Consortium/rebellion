...
Current input direction: the owner requested **controller-only play** on September 8. The owner then clarified that right-stick deflection must aim and fire. The [twin-stick checkpoint](TWIN_STICK_PLAYTEST_2026-09-08.md) uses RT thrust, LT hull ability, RB dodge and LB overload, with face-button/D-pad ammunition selection. The [manual booster checkpoint](MANUAL_BOOSTER_PLAYTEST_2026-09-08.md) adds three-dose timed-drug storage, D-pad up/down selection and Y activation. The controller-only connection screen and disconnect protection remain. The older remapping screen is replaced by a controller guide in player builds. Keyboard input remains available only to development fixtures. Physical-device qualification remains open.

Latest scoring follow-up: the [September 9 run-results checkpoint](RUN_RESULTS_PLAYTEST_2026-09-09.md) retains maximum chain and combat time across missions, compares and persists personal bests on terminal results, and clears old score on new attempts. The Linux package linked in the manual-booster checkpoint predates these changes.
...
| RBL-05 / P1 | Qualify the owner's controller-only direction; see the [controller and booster checkpoint](MANUAL_BOOSTER_PLAYTEST_2026-09-08.md) | Fixed controls reach every offered route; right-stick aiming fires in the same direction; trigger/bumper utilities remain distinct; old bindings cannot interfere; disconnect freezes play and reconnection requires acknowledgement; stored doses survive mission continuation and pause, reset on death/restart, and cannot be spent accidentally by held controls; physical controller and feedback checks pass |
| RBL-06 / P1 | Implemented in the [run-results checkpoint](RUN_RESULTS_PLAYTEST_2026-09-09.md): retained best chain and combat time, pre-save personal-best comparison, compatible record metadata and attempt reset boundaries | Regression and rendered evidence are linked in the checkpoint; physical retry/results acceptance and updated Linux qualification remain open |
