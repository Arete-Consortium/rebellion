...
Current input direction: the owner requested **controller-only play** on September 8. The owner then clarified that right-stick deflection must aim and fire. The [twin-stick checkpoint](TWIN_STICK_PLAYTEST_2026-09-08.md) uses RT thrust, LT hull ability, RB dodge and LB overload, with face-button/D-pad ammunition selection. The controller-only connection screen and disconnect protection remain. The older remapping screen is replaced by a controller guide in player builds. Keyboard input remains available only to development fixtures. Physical-device qualification remains open.
...
| RBL-05 / P1 | Qualify the owner's controller-only direction; see the [controller checkpoint](TWIN_STICK_PLAYTEST_2026-09-08.md) | Fixed controls reach every offered route; right-stick aiming fires in the same direction; trigger/bumper utilities remain distinct; old bindings cannot interfere; disconnect freezes play and reconnection requires acknowledgement; physical controller and feedback checks pass |
